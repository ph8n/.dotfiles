...
- Declarative configuration lives in `~/.nix-conf`. Prefer changing that repository and running `nix flake check ~/.nix-conf` followed by `hm`; do not edit generated files in `~/.config` directly.
