...
local function default_sysroot_src()
  ...
  if not sysroot then
    ...
    if result.code == 0 and stdout then
      if stdout:sub(-1) == "\n" then
        sysroot = #stdout > 1 and stdout:sub(1, -2) or ""
      else
        sysroot = stdout
      end
    end
...
return {
  ...
  before_init = function(init_params, config)
    ...
    vim.lsp.commands["rust-analyzer.runSingle"] = function(command)
      local run = command.arguments[1]
      local cmd = { "cargo", table.unpack(run.args.cargoArgs) }
      if run.args.executableArgs and #run.args.executableArgs > 0 then
        vim.list_extend(cmd, { "--", table.unpack(run.args.executableArgs) })
      end

      local result = vim.system(cmd, { cwd = run.args.cwd, env = run.args.environment }):wait()
